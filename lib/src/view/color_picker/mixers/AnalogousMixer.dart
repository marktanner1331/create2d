...
class AnalogousMixer extends ColorMixerGroup {
  ColorSwatch _pre;
    ColorSwatch _same;
  ColorSwatch _post;

  AnalogousMixer(ColorPicker colorPicker) : super(colorPicker, "Analogous") {
    _pre = addSwatch();
    _same = addSwatch();
    _post = addSwatch();
  }
  ...
  void refreshForColor(int color) {
    ...
    if(s == 0) {
      ...
      if(v < 1/12) {
        ...
        _same.color = ColorHelper.HSVtoRGB(h, s, v + 1/6);
        _post.color = ColorHelper.HSVtoRGB(h, s, v + 2/6);
      } else if(v > 11/12) {
        _post.color = ColorHelper.HSVtoRGB(h, s, v);
        _same.color = ColorHelper.HSVtoRGB(h, s, v - 1/6);
        _pre.color = ColorHelper.HSVtoRGB(h, s, v - 2/6);
      } else {
        _pre.color = ColorHelper.HSVtoRGB(h, s, v - 1/12);
        _same.color = ColorHelper.HSVtoRGB(h, s, v);
        _post.color = ColorHelper.HSVtoRGB(h, s, v + 1/12);
      }
    } else {
      //if we do have some saturation then we can modify the hue
      _pre.color = ColorHelper.HSVtoRGB((h - 1/12) % 1, s, v);
      _same.color = ColorHelper.HSVtoRGB(h, s, v);
      _post.color = ColorHelper.HSVtoRGB((h + 1/12) % 1, s, v);
    }
  }
}